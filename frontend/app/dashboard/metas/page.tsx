  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-3xl font-bold" style={{ color: 'var(--foreground)' }}>Metas Financeiras</h1>
          <p className="mt-1" style={{ color: 'var(--muted-foreground)' }}>Defina e acompanhe seus objetivos financeiros</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {metas.map((meta) => {
          ...
          return (
            <div
              key={meta.id_meta}
              className="rounded-2xl shadow-sm hover:shadow-md transition-all duration-300 overflow-hidden relative flex flex-col justify-between"
                ...
                <div className="space-y-4">
                  <div className="flex items-start justify-between">
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        <h3 className="font-semibold text-lg" style={{ color: 'var(--foreground)' }}>{meta.nome}</h3>
                  ...
                  <div className="rounded-xl p-4 space-y-3" style={{ background: 'var(--muted)', border: '1px solid var(--border)' }}>
                    <div className="flex justify-between items-center">
                      <span className="text-sm flex items-center gap-1.5" style={{ color: 'var(--muted-foreground)' }}>
                    ...
                    {meta.status === 'Em Andamento' && (
                      <>
                        <button
                          ...
                          style={{ background: 'rgba(234,179,8,0.08)', color: 'var(--primary)', border: '1px solid rgba(234,179,8,0.2)' }}
                        ...
                        <button
                          className="flex items-center justify-center px-2 py-1.5 rounded-lg text-xs font-medium transition-all"
      ...
      {metas.length === 0 && (
        <div className="rounded-2xl shadow-sm" style={{ background: 'var(--card)', border: '1px solid var(--border)' }}>
          <div className="text-center py-16">
            <div className="mx-auto w-16 h-16 rounded-2xl flex items-center justify-center mb-4" style={{ background: 'var(--muted)', border: '1px solid var(--border)' }}>
              <Target className="w-8 h-8 text-yellow-500" />
            </div>
            <p className="text-lg" style={{ color: 'var(--foreground)' }}>Nenhuma meta cadastrada</p>
            <p className="text-sm mt-1" style={{ color: 'var(--muted-foreground)' }}>Crie sua primeira meta financeira para começar</p>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4 overflow-y-auto">
          <div className="rounded-2xl w-full max-w-md overflow-hidden my-8" style={{ background: 'var(--card)', border: '1px solid var(--border)', boxShadow: 'var(--glass-shadow)' }}>
            <div className="px-6 py-5" style={{ borderBottom: '1px solid var(--border)', background: 'var(--muted)' }}>
            ...
            <div className="p-6">
              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  <label className="text-sm font-medium" style={{ color: 'var(--foreground)' }}>Nome da Meta</label>
                  <Input
                    value={formData.nome}
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="text-sm font-medium" style={{ color: 'var(--foreground)' }}>Valor Alvo (R$)</label>
                    <Input
                      type="number"
                      step="0.01"
                  ...
                  <div>
                    <label className="text-sm font-medium" style={{ color: 'var(--foreground)' }}>Valor Inicial (R$)</label>
                    <Input
                      type="number"
                      step="0.01"
                      ...
                      style={{ border: '1px solid var(--border)', background: 'var(--muted)', color: 'var(--foreground)' }}
                    />
                    <p className="text-xs mt-1" style={{ color: 'var(--muted-foreground)' }}>Quanto já guardou</p>
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="text-sm font-medium" style={{ color: 'var(--foreground)' }}>Data de Início</label>
                    <Input
                      type="date"
                  ...
                  <div>
                    <label className="text-sm font-medium" style={{ color: 'var(--foreground)' }}>Previsão (Opcional)</label>
                    <Input
                      type="date"
      ...
      {showAddValueModal && selectedMeta && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4 overflow-y-auto">
          <div className="rounded-2xl w-full max-w-sm overflow-hidden my-8" style={{ background: 'var(--card)', border: '1px solid var(--border)', boxShadow: 'var(--glass-shadow)' }}>
            <div className="px-6 py-5" style={{ borderBottom: '1px solid var(--border)', background: 'var(--muted)' }}>
              <h3 className="text-xl font-bold flex items-center gap-2" style={{ color: 'var(--foreground)' }}>
                <DollarSign className="w-5 h-5 text-yellow-500" />
                Adicionar Valor
              </h3>
              <p className="text-sm mt-1" style={{ color: 'var(--muted-foreground)' }}>{selectedMeta.nome}</p>
            </div>
            <div className="p-6 space-y-4">
              {/* Current Progress */}
              <div className="rounded-xl p-4 space-y-2" style={{ background: 'var(--muted)', border: '1px solid var(--border)' }}>
                ...
                <div className="w-full rounded-full h-2 mt-2" style={{ background: 'var(--card)', border: '1px solid var(--border)' }}>
                  <div
              ...
              {/* Add Value Input */}
              <div>
                <label className="text-sm font-medium" style={{ color: 'var(--foreground)' }}>Valor a adicionar (R$)</label>
                <Input
                  type="number"
                  step="0.01"
                  min="0.01"
              ...
              {addValueAmount && parseFloat(addValueAmount) > 0 && (
                <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-xl p-3">
                  <p className="text-sm text-yellow-500">
                    ...
                    <span style={{ color: 'var(--foreground)' }}>{formatCurrency(parseFloat(String(selectedMeta.valor_atual) || '0') + parseFloat(addValueAmount))}</span>
                    {' / '}
                    <span style={{ color: 'var(--muted-foreground)' }}>{formatCurrency(parseFloat(String(selectedMeta.valor_alvo)))}</span>
                  </p>
